	...
	go func() {
		for {
			...
			case job := <-w.JobQueue:
				...
				fmt.Println("")
			case <-w.QuitChan:
				fmt.Printf("Worker with id %d Stopped", w.Id)
				fmt.Println("")
			}
		}
	}()
...
func ResquestHandler(w http.ResponseWriter, r *http.Request, jobQueue chan Job) {
	if r.Method != "POST" { //GET,PUT,DELET
